...
import { seededConnections } from "../telemetry/connections.js";
...
/** Handle `/api/orgs/:orgId/envs/:envId/connections`. Caller has authenticated `user`. */
export async function handleConnectionsRequest(
  ...
  const orgId = segs[0];
  const envId = segs[2];
  if (!orgId || segs[1] !== "envs" || !envId || segs[3] !== "connections") {
  ...
  try {
    ...
    return okResponse(seededConnections(env.id, env.mode, now));
  } catch (e) {
