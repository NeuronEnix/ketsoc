...
import { seededEvents } from "../telemetry/events.js";
...
/** Handle `/api/orgs/:orgId/envs/:envId/events`. Caller has authenticated `user`. */
export async function handleEventsRequest(
  ...
  const orgId = segs[0];
  const envId = segs[2];
  if (!orgId || segs[1] !== "envs" || !envId || segs[3] !== "events") {
  ...
  try {
    ...
    return okResponse(seededEvents(env.id, env.mode, now));
  } catch (e) {
